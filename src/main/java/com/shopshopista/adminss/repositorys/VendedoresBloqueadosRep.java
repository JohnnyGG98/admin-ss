...
public interface VendedoresBloqueadosRep extends JpaRepository<VendedoresBloqueados, Long>  {
    ...
    @Query(value = "SELECT v FROM vendedores_bloqueados v WHERE v.id_vendedor_bloqueado = :idVenBloqueado")
    List<VendedoresBloqueados> buscarPorIdVenBloqueados(@Param("idVenBloqueado") Long idVenBloqueado);
    
    @Query(value = "UPDATE vendedores_bloqueados v SET v.vebl_activo = false WHERE v.id_vendedor_bloqueado = :idVenBloqueado")
